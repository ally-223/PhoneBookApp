...
//      console.log('Adding item:', newName, newNum); 
//     const id = uuidv4();
//     const nameObject = {
//       id,
//       name: newName,
...
const App = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then(response => {
        setPersons(response.data);
        // Set nextId to the highest current ID + 1
        const maxId = Math.max(...response.data.map(person => person.id), 0);
        setNextId(maxId + 1);
      })
      .catch(error => {
        console.error('Failed to fetch persons:', error);
  ...
  const addItem = (event) => {
    ...
    noteService
      ...
      .then(response => {
        ...
        setNewName('');
        setNewNum('');
        setNextId(nextId + 1); // Increment nextId for the next new item
      })
      .catch(error => {
        console.error('Failed to add new item:', error);
      });
